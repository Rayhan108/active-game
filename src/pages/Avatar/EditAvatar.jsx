...
export default function EditAvatar({ avatar, onClose }) {
  const {
    register,
    handleSubmit,
    ...
    defaultValues: {
      ...
      rotScore: avatar.rotLimit.toString(),
    },
  })

  const [imagePreview, setImagePreview] = useState(avatar.image || null)
  const [isGoldenAvatar, setIsGoldenAvatar] = useState(
    avatar.isGolden || avatar.name.toLowerCase() === "golden"
  )
  ...
  const onFormSubmit = (data) => {
    ...
    console.log("Edit Avatar Form Submitted:", submitData)
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-xl p-5 w-full">
      {/* Header */}
      <div className="mb-4">
        <h2 className="text-white text-lg font-medium">Edit Avatar</h2>
        <p className="text-gray-400 text-sm">
          Editing: <span className="text-amber-400">{avatar.name}</span>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onFormSubmit)}>
          ...
          <div className="flex-1 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                {...register("name")}
                className="w-full px-3 py-2.5 rounded-lg bg-[#2a2a2a] border border-[#3a3a3a] text-white placeholder-[#555] text-sm focus:outline-none focus:border-amber-500 transition-colors"
              />
            </div>
        ...
        <div className="flex justify-end gap-3 mt-5">
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2.5 rounded-lg bg-[#333] hover:bg-[#444] text-white text-sm font-medium transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium transition-colors disabled:opacity-50"
          >
            Update Avatar
          </button>
        </div>
